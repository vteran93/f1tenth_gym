...
def main():
    ...
    def predict_action(model, obs_np, agent_idx):
        if np.isnan(obs_np).any() or np.isinf(obs_np).any():
            print(f"[ERROR] Obs for agent {agent_idx} has NaN or Inf:", obs_np)
            obs_np = np.nan_to_num(obs_np, nan=0.0, posinf=0.0, neginf=0.0)
        try:
            action, _ = model.predict(obs_np, deterministic=True)
            if np.isnan(action).any() or np.isinf(action).any():
                print(
                    f"Agent {agent_idx} - Action: {action} | Predict time: {time.time() - start:.4f}s")

                print(
                    f"[ERROR] Action from agent {agent_idx} is NaN or Inf:", action)
                action = np.zeros_like(action)
        except Exception as e:
            print(
                f"[ERROR] Failed to predict action for agent {agent_idx}: {e}")
            action = np.zeros((agent_envs[agent_idx].action_space.shape[0],))
    ...
    while not done and step < max_steps:
        ...
        obs_batch = [agent_envs[i].extract_obs(
            obs_full) for i in range(num_agents)]

        start = time.time()
        with ThreadPoolExecutor(max_workers=num_agents) as executor:
            futures = [executor.submit(
                predict_action, models[i], obs_batch[i], i) for i in range(num_agents)]
